            ...
            for attempt in range(10):
                ...
                if self.consistent:
                    ...
                    if w <= img1.size[0] and h <= img1.size[1]:
                        ...
                        return [
                            i.resize((self.size, self.size),
                                     self.interpolation) for i in imgmap
                        ]
                else:
                    result = []
    ...
    def __call__(self, imgmap):
        if random.random() < self.threshold:  # do RandomRotation
            if self.consistent:
                ...
                return [
                    i.rotate(np.random.randint(-self.degree, self.degree,
                                               1)[0],
                             expand=True) for i in imgmap
                ]
...
class OneClipTransform:
    ...
    def __call__(self, x):
        # target: list of image
        # if len(x) != 2 * self.seq_len:
        #     print(len(x), self.seq_len)
        assert len(x) == 2 * self.seq_len
...
class TransformController:
    ...
    def __call__(self, x):
        idx = random.choices(range(self.num_transform),
                             weights=self.weights)[0]
        return self.transform_list[idx](x)

    def __str__(self):
        string = 'TransformController: %s with weights: %s' % (str(
            self.transform_list), str(self.weights))
        return string
