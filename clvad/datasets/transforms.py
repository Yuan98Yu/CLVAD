...
def rgb_to_grayscale(vid, channel=0):
    ...
    return (0.2989 * vid.select(channel, 0) + 0.5870 * vid.select(channel, 1) +
            0.1140 * vid.select(channel, 2)).to(vid.dtype)
...
def adjust_contrast(vid, contrast_factor, channel=1, gray_channel=0):
    ...
    # mean: [N,1,1]
    mean_gray = rgb_to_grayscale(vid,
                                 gray_channel).to(torch.float)  # not mean yet
...
def random_adjust_brightness(vid, brightness_factor, consistent, channel=1):
    ...
    if consistent:
        ...
        brightness_factor = np.random.uniform(brightness_factor[0],
                                              brightness_factor[1],
                                              size=(N, ))
...
def random_adjust_contrast(vid,
                           contrast_factor,
                           consistent,
                           channel=1,
                           gray_channel=0):
    ...
    if consistent:
        ...
        contrast_factor = np.random.uniform(contrast_factor[0],
                                            contrast_factor[1],
                                            size=(N, ))
...
def random_adjust_saturation(vid,
                             saturation_factor,
                             consistent,
                             channel=1,
                             gray_channel=0):
    ...
    if consistent:
        ...
        saturation_factor = np.random.uniform(saturation_factor[0],
                                              saturation_factor[1],
                                              size=(N, ))
...
class ColorJitter(object):
    ...
    @staticmethod
    def get_params(brightness, contrast, saturation, consistent, n_channel,
                   gray_channel):
    ...
    def __call__(self, vid):
        if random.random() < self.p:
            transform = self.get_params(self.brightness, self.contrast,
                                        self.saturation, self.consistent,
                                        self.n_channel, self.gray_channel)
